...
for agent_id in agents:
    ...
    if os.path.exists(f"{agent_id}.pth"):
        agent_instance.action_value_function.load_state_dict(torch.load(f"{agent_id}.pth"))
...
for ep in range(1, NUM_OF_EPISODES + 1):
    ...
    if ep % NUM_OF_STEPS_TO_UPDATE == 0:
        for agent_id in agents:
            torch.save(agent_models[agent_id].action_value_function.state_dict(), f"{agent_id}.pth")
